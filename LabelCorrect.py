...
def train_correction_zhao(model, warmup_dataloader,update_dataloader, optimizer, device, 
    ...
    if epoch < warmup_epoch:
        ...
        from Utils.loss import JSD_Mult_SmoothLoss      
        criterion = JSD_Mult_SmoothLoss(
                    smoothing=0.1,
                    threshold=0.9,
                    ce_weight=1.0,
        ...
        model.train()
        from Utils.loss import JSD_Mult_SmoothLoss, EdgeTanimotoLoss
        criterion_ori = JSD_Mult_SmoothLoss(
            smoothing=0.1,
            threshold=0.9,
            consistency_weight=1.0,
            ce_weight=1.0,
        ...
        final_dataset = CAOISPRSDataset(
            image_dir=args.train_image_folder,
            label_dir=os.path.join(args.save_dir,'update_label'),
